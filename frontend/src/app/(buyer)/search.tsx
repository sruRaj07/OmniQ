...
function SearchSvg({ color = "#333", size = 22 }) {
  return (
...
function ArrowLeftSvg({ color = colors.textPrimary, size = 24 }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function ClockSvg({ color = colors.textMuted, size = 16 }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function TrendingSvg({ color = colors.accentLight, size = 16 }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function CloseSvg({ color = colors.textMuted, size = 18 }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function usePopularProducts() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const { data } = await apiClient.get("/products", { params: { limit: 6 } });
...
export default function SearchScreen() {
  ...
  const { products: popularProducts, isLoading: isPopularLoading } = usePopularProducts();
  ...
  const handleCategoryFilter = useCallback(
    (cat: string) => {
      ...
      setFilters((f) => ({ ...f, category: newCat }));
  ...
  return (
    ...
    >
        ...
        <View style={styles.idleContainer}>
            ...
            <View style={styles.section}>
              ...
              {recentSearches.map((term) => (
                ...
                >
                  ...
                  <TouchableOpacity onPress={() => removeRecentSearch(term)} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
          ...
          {dbCategories.length > 0 && (
            <View style={styles.section}>
              <View style={styles.sectionHeader}>
                <TrendingSvg />
                <Text style={[styles.sectionTitle, { marginLeft: 8 }]}>Browse Categories</Text>
              </View>
      ...
      {isTyping && (
        <TouchableOpacity style={styles.searchForRow} onPress={handleSubmit}>
          <SearchSvg color={colors.accentLight} size={18} />
          <Text style={styles.searchForText}>
            Search for "<Text style={{ fontWeight: "800", color: colors.textPrimary }}>{query}</Text>"
          </Text>
        </TouchableOpacity>
      )}
          ...
          <View style={styles.filterBar}>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterScroll}>
              {dbCategories.map((cat) => (
                <TouchableOpacity
                  key={cat}
                  style={[styles.filterPill, filters.category === cat && styles.filterPillActive]}
