...
export default function SellerDashboardScreen() {
  ...
  const recentOrders = [...sellerOrders]
    ...
    .slice(0, 5);
  const activeOrders = sellerOrders.filter((o: any) => o.status !== 'cancelled');
  const todayOrders = activeOrders.filter((o: any) => new Date(o.created_at) >= today);
  const yesterdayOrders = activeOrders.filter((o: any) => {
  ...
  const revenueTrendText = revenueYesterday === 0 && revenueToday === 0 ? "No revenue yet" : revenueYesterday === 0 ? "↑ 100% vs yesterday" : revenueTrend >= 0 ? `↑ ${revenueTrend}% vs yesterday` : `↓ ${Math.abs(revenueTrend)}% vs yesterday`;
  const newOrdersToday = todayOrders.length;
  const newOrdersYesterday = yesterdayOrders.length;
  const ordersTrendText = newOrdersYesterday === 0 && newOrdersToday === 0 ? "No orders yet" : newOrdersToday >= newOrdersYesterday ? `↑ ${newOrdersToday - newOrdersYesterday} vs yesterday` : `↓ ${newOrdersYesterday - newOrdersToday} vs yesterday`;
  const liveProductsCount = products.filter((p: any) => p.is_active).length;
  const outOfStock = products.filter((p: any) => p.stock_quantity === 0).length;
  const pendingApproval = products.filter((p: any) => !p.is_active).length;
  ...
  const dailyRevenue = last7Days.map(date => {
    ...
    const dayOrders = activeOrders.filter((o: any) => {
      const d = new Date(o.created_at);
  ...
  const dayNames = last7Days.map(d => d.toLocaleDateString("en-US", {
    weekday: "short"
  }).charAt(0));
  return <>
      ...
      label: "Profile"
        ...
        alignItems: "center"
            ...
            <View style={styles.kpiGrid}>
              ...
              <KpiCard label="PRODUCTS LIVE" value={liveProductsCount.toString()} trend="Currently approved" />
              <KpiCard label="AVG. RATING" value="N/A" trend="No reviews yet" />
            </View>
            <Card style={styles.chart}>
              ...
              <View style={styles.bars}>
                {normalizedBars.map((height, index) => <View key={index} style={[styles.bar, {
              height
            }]} />)}
              </View>
              <View style={styles.days}>
                {dayNames.map((day, index) => <Text key={`${day}-${index}`} style={styles.day}>{day}</Text>)}
              </View>
            </Card>
            ...
            {sellerOrders.length === 0 ? <Text style={{
          color: colors.textSecondary,
          textAlign: "center",
          paddingVertical: 20
        }}>No recent orders.</Text> : <View style={styles.ordersList}>
          {recentOrders.map((order: any) => <OrderCard key={order.id} order={order} />)}
        </View>}
          </>}
      </Screen>
